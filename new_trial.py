...
lower_r=np.array([140, 20, 20])
upper_r=np.array([255, 200, 200])
lower_g=np.array([40, 40, 72])
upper_g=np.array([102, 255, 255]) #40,80,150]) upper_g=np.array([255,255,255])
...
while True:
    ...
    if(b-a>2):
        ...
        if m==10:
            cv2.putText(imga,'MOVED RIGHT',(50,50),cv2.FONT_HERSHEY_SIMPLEX, fontScale = 1,color = (0, 0, 0),thickness = 2)
        elif m==5:
            cv2.putText(imga,'MOVED LEFT',(50,50),cv2.FONT_HERSHEY_SIMPLEX, fontScale = 1,color = (0, 0, 0),thickness = 2)
        else:
            cv2.putText(imga,'DID NOT MOVE',(50,50),cv2.FONT_HERSHEY_SIMPLEX, fontScale = 1,color = (0, 0, 0),thickness = 2)
        if c==10:
            cv2.putText(imga,'OPENED',(70,70),cv2.FONT_HERSHEY_SIMPLEX, fontScale = 1,color = (255, 255, 255),thickness = 2)
        else:
           cv2.putText(imga,'CLOSED',(70,70),cv2.FONT_HERSHEY_SIMPLEX, fontScale = 1,color = (255, 255, 255),thickness = 2)
        movs=[]
        closs=[]
